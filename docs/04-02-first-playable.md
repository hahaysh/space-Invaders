...
### 예정 명령을 실제 실행 근거와 연결하기

01-03에서는 명령을 추후 확인하도록 했고, 03-02에서는 TRD의 예정 명령을 지침에 반영했습니다.
이제 처음 생성·실행한 `package.json`과 비교해 그 약속을 마무리합니다.
**Plan**에서 확인하고, 필요한 지침 변경만 따로 승인합니다.

```text
AGENTS.md·TRD.md의 예정 명령과 실제 package.json·M1 실행 결과를 읽기 전용으로 비교해줘.
dev/build/preview/test/test:e2e의 정의와 실제 실행/미실행을 구분해줘.
E2E의 5173 관리 서버와 별도 수동 preview의 4173을 혼동하지 않았는지 봐줘.
AGENTS.md에 남은 추후 확인 문구나 실제 명령 불일치가 있으면 최소 수정안을 제시해줘.
같은 정의가 이미 정확하다면 불필요한 수정은 제안하지 마.
아직 실행하지 않은 preview 등은 검증 완료라고 바꾸지 말고 코드는 수정하지 마.
```

수정안이 있다면 **Interactive**에서 “검토한 명령 안내 보완만 승인한다. `AGENTS.md`에 반영하고 다시 읽어 적용 규칙을 요약해줘. 실행 로그는 복제하지 말고 diff를 보여준 뒤 커밋 없이 멈춰줘”라고 요청합니다.
문제없는 지침은 그대로 두고 대화에 확인 근거만 남깁니다. 자동 지침 로딩 여부와 명령 실행 확인도 구분합니다.

**Interactive**에서 아래 결과 기록을 승인합니다.
...
계획과 필요한 지침 diff까지 확인한 뒤 “검토한 M1 파일과 별도로 승인한 AGENTS.md 변경만 커밋하고 푸시하지 마. 메시지는 너무 짧지 않은 한국어 설명형 제목과 본문으로 작성하고, 본문에 04-02/M1 단계 ID, 변경 내용과 이유, 실제 검증 결과와 미확인 범위를 적어줘”라고 요청합니다.
