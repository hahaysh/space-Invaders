...
test('TC-P01 P는 playing과 paused만 전환하고 제목·종료에서 무시한다', () => {
  const game = playing();
  assert.equal(press(game, 'KeyP').status, 'paused');
  assert.equal(press(press(game, 'KeyP'), 'KeyP').status, 'playing');
  for (const status of ['title', 'won', 'lost']) {
    const inactive = createGame(status);
    assert.strictEqual(press(inactive, 'KeyP'), inactive);
    assert.deepEqual(inactive, createGame(status));
  }
});

test('TC-P01 반복 P와 P keyup은 한 번 더 전환하지 않는다', () => {
  for (const status of ['playing', 'paused']) {
    const game = createGame(status);
    assert.strictEqual(keyInput(game, 'KeyP', true, true), game);
    assert.strictEqual(keyInput(game, 'KeyP', false), game);
  }
});

test('TC-P02 정지 중 긴 dt에도 모든 값과 쿨다운·게임 시간이 그대로다', () => {
  let game = step(press(playing(), 'Space'), 0.1);
  game = press(press(game, 'ArrowRight'), 'KeyP');
  const snapshot = structuredClone(game);
  close(snapshot.cooldown, 0.1);
  close(snapshot.time, 0.1);
  assert.equal(snapshot.bullets.length, 1);
  assert.deepEqual(snapshot.keys, {});
  for (const dt of [0, 0.1, 60, 3600]) {
    const next = step(game, dt);
    assert.strictEqual(next, game);
    assert.deepEqual(next, snapshot);
    assert.deepEqual(next.player, snapshot.player);
    assert.deepEqual(next.enemies, snapshot.enemies);
    assert.deepEqual(next.bullets, snapshot.bullets);
    assert.equal(next.score, snapshot.score);
    assert.equal(next.cooldown, snapshot.cooldown);
    assert.equal(next.time, snapshot.time);
    assert.equal(next.nextBulletId, snapshot.nextBulletId);
  }
});

test('TC-P03 paused의 이동·발사·Enter·R은 입력에 쌓이지 않는다', () => {
  const paused = press(playing(), 'KeyP');
  for (const code of ['ArrowLeft', 'ArrowRight', 'KeyA', 'KeyD', 'Space', 'Enter', 'KeyR']) {
    assert.strictEqual(press(paused, code), paused);
  }
  assert.strictEqual(command(paused, 'start'), paused);
  assert.strictEqual(command(paused, 'restart'), paused);
  const resumed = step(press(paused, 'KeyP'), 0.1);
  assert.equal(resumed.player.x, 380);
  assert.equal(resumed.bullets.length, 0);
  assert.deepEqual(resumed.keys, {});
});

test('TC-P03 정지·재개 전환은 키를 비우고 반복 이벤트로 되살리지 않는다', () => {
  const initial = press(press(playing(), 'ArrowRight'), 'Space');
  const before = structuredClone(initial);
  const paused = press(initial, 'KeyP');
  assert.deepEqual(initial, before);
  assert.deepEqual(paused.keys, {});
  let resumed = press(paused, 'KeyP');
  assert.deepEqual(resumed.keys, {});
  resumed = keyInput(resumed, 'ArrowRight', true, true);
  resumed = keyInput(resumed, 'Space', true, true);
  assert.equal(step(resumed, 0.1).player.x, 380);
  assert.equal(step(resumed, 0.1).nextBulletId, 0);
  resumed = press(release(resumed, 'ArrowRight'), 'ArrowRight');
  close(step(resumed, 0.1).player.x, 412);
});

test('TC-P04 정지 60초 후에도 남은 0.1초 쿨다운 경계를 정확히 보존한다', () => {
  let game = step(press(playing(), 'Space'), 0.1);
  game = step(press(game, 'KeyP'), 60);
  game = press(game, 'KeyP');
  close(game.time, 0.1);
  close(game.cooldown, 0.1);
  assert.equal(game.nextBulletId, 1);
  game = step(press(game, 'Space'), 0.1 - 1e-10);
  assert.equal(game.nextBulletId, 1);
  game = step(game, 1e-10);
  assert.equal(game.nextBulletId, 2);
  close(game.bullets.at(-1).bornAt, 0.2, 1e-12);
...
test('TC-P07 blur 입력 해제는 playing과 paused의 상태·시간을 바꾸지 않는다', () => {
  for (const status of ['playing', 'paused']) {
    const game = { ...createGame(status), keys: { Space: true }, cooldown: 0.15, time: 1.25 };
    const before = structuredClone(game);
    const next = clearInput(game);
    assert.deepEqual(next, { ...before, keys: {} });
    assert.deepEqual(game, before);
