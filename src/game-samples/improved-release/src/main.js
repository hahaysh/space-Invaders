...
const labels = { title: '출격 대기', playing: '방어 중', paused: '일시정지', won: '방어 성공', lost: '방어선 돌파' };
const controlled = new Set(['ArrowLeft', 'ArrowRight', 'KeyA', 'KeyD', 'Space', 'Enter', 'KeyR', 'KeyP']);
let game = createGame();
...
function updateDom() {
  ...
  if (game.status === 'paused') {
    badge.textContent = '작전 대기 / 일시정지';
    message.textContent = '잠시 쉬어 가세요';
    detail.textContent = '기체와 적, 탄환이 멈췄어요. 준비가 되면 이어서 도전하세요.';
    shortcut.textContent = 'P 키로 재개 · 이동과 발사는 다시 눌러 주세요';
  }
