...
최초 체크포인트 `0162756`의 설계를 보존하고 CHG-01 관련 부분만 확장한다.
...
| `src/game.js` | 순수 상태 생성·명령·P 전환·키 처리·충돌·시간 갱신 | 01~07, CHG-01-AC1/2/4/5 |
| `src/main.js` | DOM 연결, 키·포커스 이벤트, 단일 RAF, 정지 안내·도형 렌더링 | 01~03, 06~08, CHG-01-AC3~6 |
...
발사 대기시간, 다음 탄환 번호로 구성한다. title/paused/종료 상태의 step은 같은 상태를 반환한다.
`keyInput`은 기존 반복 차단 뒤 P를 처리해 playing/paused만 서로 전환하고 키 집합을 비운다.
쿨다운·시간·위치는 복원용 별도 복사본을 만들지 않고 같은 값으로 유지한다.
...
시작·재시작·일시정지·재개는 RAF를 만들지 않는다. RAF timestamp 차이를 초로 바꿔 step에 넘긴다.
...
CHG-01은 기존 `accept`의 모든 상태 전환 처리(`clearInput`, `previousTime = null`)를 그대로 재사용한다.
paused에서도 RAF는 그림을 다시 그리지만 step은 시간을 진행하지 않는다.
재개 첫 RAF는 dt=0이며 상한만으로 큰 dt를 숨기는 방식이 아니다.
정지 UI는 DOM 상태·overlay의 제목·안내만 갱신하고 시작/재시작 버튼은 둘 다 숨긴다.
playing 복귀는 overlay를 숨기며 다음 won/lost는 종료 문구를 새로 채워 이전 정지 안내가 남지 않는다.
...
임의의 사용 가능한 loopback 포트의 `/improved-release/` 아래에 제공하고 Chromium으로
상대 JS/CSS 요청·시작 버튼·P 정지/재개·상태·점수·오류를 검사한 뒤 서버와 브라우저를 닫는다.
...
이상은 최초 설계 당시의 기록이다. 실제 최초 결과는 `FIRST_RELEASE_RESULTS.md`에 보존한다.

2026-09-13 최초 체크포인트 이후 사용자가 CHG-01 개선판의 문서·구현·로컬 검증을 허용했다.
이번 코드 작성 전에 위 상태·입력·시간·표시 설계를 갱신했다.
개선 결과는 `TEST_RESULTS.md`에 별도로 기록하며 참가자별 승인·공개 배포를 주장하지 않는다.
기존 의존성·잠금 파일·CI·CSS는 그대로 사용한다.
