...
[실습 전체 목차](../../docs/00-전체-실습-안내.md) · [출처와 확인 범위](../../REFERENCES.md)

이 폴더에는 이전 실습의 **최초 버전과 일시정지 개선 버전**을 역사적 참고 프로젝트로 보관합니다. 현재 v2는 전체 안내 1개와 실습 20개이며, 참가자는 빈 저장소에서 **우주 방어**를 만듭니다. 샘플을 복사해서 시작하지 않으며 샘플의 기존 제목·코드·테스트는 바꾸지 않습니다.

> 두 샘플은 구현·로컬 자동 검증을 마쳤으며 해당 샘플의 원격 배포 근거는 없습니다. 별도 demo01은 이전 25개 실습, **v2/demo02는 20개 단계의 위임 실행과 실제 공개 배포**를 완료했습니다. 사람의 전체 App UI 확인과는 구분합니다.

이후 추가한 [카드형 상태바·비행체 에셋 기준](../../docs/화면과-에셋-기준.md)은 아래 샘플과 demo01·demo02에 적용하지 않았습니다. 이 시각 보완의 전체 게임 리허설도 아직 하지 않았으므로 이전 화면·검사 수를 새 기준의 근거로 사용하지 않습니다.
...
[08 난이도 선택](../../docs/08-01-난이도-선택-설계.md)과 [09 목숨](../../docs/09-01-목숨-설계.md)는 일시정지 이후의 선택 실습입니다. 두 샘플에는 이 기능이 없으며 아래 샘플 테스트 수를 확장 기능의 근거로 사용하지 않습니다. 확장까지 완료한 별도 demo01과 현재 v2의 새 리허설 상태를 구분합니다.

### 별도 demo01과 v2

- 이전 25개 실습 리허설의 [실제 공개 게임](https://hahaysh.github.io/space-Invaders-demo01/)은 난이도·목숨까지 포함합니다.
- 최종 main은 `97cd3e1fca2face6bf84c034412ae31a60fa6056`입니다.
- [Actions 34793874053](https://github.com/hahaysh/space-Invaders-demo01/actions/runs/34793874053): Node 33·Chromium 28·빌드·배포 성공.
- 자율 API 실행·직접 fast-forward 병합 예외를 사용했습니다. 사람의 App/PR UI와 AGENTS 자동 로딩은 미검증입니다.
- 이전 고정 안내서 [`c545b103`](https://github.com/hahaysh/space-Invaders/tree/c545b103/docs)는 역사적 자료입니다. 현재 v2/demo02의 20개 실습 성공을 입증하지 않습니다.

v2의 별도 [demo02 공개 게임](https://hahaysh.github.io/space-Invaders-demo02/)은 새 이슈·기능 세션·PR 흐름으로 난이도와 목숨까지 구현했습니다. [최종 배포](https://github.com/hahaysh/space-Invaders-demo02/actions/runs/34821940747)는 Node 34·Chromium 17·빌드·배포가 성공했고, [단계별 진행 기록](../../reports/demo02-progress.md)에 실제 공개 확인과 App UI 미확인을 구분했습니다. 이 결과가 두 역사적 샘플의 기능이나 검사 수를 바꾸지는 않습니다.

demo01·demo02와 두 샘플의 게임 코드·테스트는 이번 문서 개편의 수정 대상이 아닙니다.
